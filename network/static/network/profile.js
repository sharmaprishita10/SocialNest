document.addEventListener('DOMContentLoaded', () => {
    // For displaying the no. of followers and following
    follow_count();

    // To load all posts by a specific user
    const username = document.querySelector('#user_posts_div').dataset.username;
    view_posts(username , 'user_posts_div', 1)
    ...
    if(follow_btn != null) {
        ...
        follow_btn.onclick = () => {
            const action = follow_btn.innerHTML;
            console.log(action);
...
function follow_count() {
    ...
    const username = document.querySelector('#follow_div').dataset.username;
    ...
    .then(data => {
        ...
        document.querySelector('#following_count').innerHTML = data.following_count;
    })
    .catch(error => {
        console.log('Error', error);
    });
...
function view_posts(type, div_name, page_no) {
    ...
    .then(posts => {
        ...
        // Create a div for each post starting from 1
